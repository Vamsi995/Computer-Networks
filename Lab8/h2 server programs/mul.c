...
void mulOperation(char *message, char *output, char *tcp_port_no);
...
{
    /* TCP Server
     Creating a TCP server on the PORT(12502)
        Create socket
    ...
    */
    ...
    while ((clientfd = accept(sockfd, (struct sockaddr*)&client_addr, &client_size)) >= 0)
    {
        ...
        mulOperation function takes this message and returns the output(num1 * num2) and the port_no(PORT)
        */
        char output[1000] = {};
        char port_no[100] = {};
        mulOperation(client_message, output, port_no);
        ...
        // Sending the MUL operation output to h3
