...
void addOperation(char *message, char *output, char *tcp_port_no);
...
void addOperation(char *message, char *output, char *tcp_port_no)
{
    ...
    token = strtok(NULL, "|");
    int second_num = atoi(token); // second_num = num2
...
int main()
{
    ...
    /* TCP Server
     Creating a TCP server on the PORT(12500)
        Create socket
    ...
    */
    ...
    while ((clientfd = accept(sockfd, (struct sockaddr*)&client_addr, &client_size)) >= 0)
    {
        ...
        addOperation function takes this message and returns the output(num1 + num2) and the port_no(PORT)
        */
        char output[1000] = {};
        char port_no[100] = {};
        addOperation(client_message, output, port_no);
        ...
        // Sending the ADD operation output to h3
