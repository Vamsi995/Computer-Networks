...
void subOperation(char *message, char *output, char *tcp_port_no);
...
int main()
{
    ...
    /* TCP Server
     Creating a TCP server on the PORT(12501)
        Create socket
    ...
    */
    ...
    while ((clientfd = accept(sockfd, (struct sockaddr*)&client_addr, &client_size)) >= 0)
    {
        ...
        subOperation function takes this message and returns the output(num1 - num2) and the port_no(PORT)
        */
        char output[1000] = {};
        char port_no[100] = {};
        subOperation(client_message, output, port_no);
        ...
        // Sending the SUB operation output to h3
